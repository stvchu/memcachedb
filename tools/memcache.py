...
client module for memcachedb based on memcached python client

Overview
========

See U{the Memcachedb homepage<http://memcachedb.googlecode.com>} for more detail.
...
class Client(local):
    ...
    def db_checkpoint(self):
        'Do a checkpoint of BDB'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('db_checkpoint')
            return(s.expect("OK") == "OK")

    def rep_ismaster(self):
        'return if it is a master'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_ismaster')
            return(s.readline())

    def rep_whoismaster(self):
        'return who is a master'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_whoismaster')
            return(s.readline())

    def rep_set_priority(self, priority):
        'set priority of replication site'
        for s in self.servers:
            if not s.connect(): continue
    ...
    def rep_set_ack_policy(self, ack_policy):
        'set ack policy of replication site'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_set_ack_policy %d' % ack_policy)
            return(s.readline())

    def rep_set_ack_timeout(self, ack_timeout):
        'set ack timeout of replication site'
        for s in self.servers:
            if not s.connect(): continue
            s.send_cmd('rep_set_ack_timeout %d' % ack_timeout)
            return(s.readline())

    def rep_set_request(self, req_min, req_max):
        'set missing log request'
        for s in self.servers:
            if not s.connect(): continue
    ...
    def delete(self, key, time=0):
        ...
        try:
            server.send_cmd(cmd)
            return(server.expect("DELETED") == "DELETED")
        except socket.error, msg:
            server.mark_dead(msg[1])
        return 0
